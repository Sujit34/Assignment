...
public class Employee {
	...
	public static void main(String[] args) {
		...
		Comparator<Employee> compareByNameThenSalary = Comparator.comparing((Employee e) -> e.getName())
				                                       .thenComparing((Employee e) -> e.getSalary(),Comparator.reverseOrder());
		List<Employee> sortedEmployees = list.stream()
                							 .sorted(compareByNameThenSalary)
                                             .collect(Collectors.toList());
		
		sortedEmployees.forEach(x->System.out.println(x.name + "---" +x.salary));
	}
}
